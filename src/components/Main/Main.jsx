...
function Main({index=0}) {
  ...
  function previousCard(){
    if(cardIndex === 0){setCardIndex((context.length -1))}
    else {
  ...
  function nextCard() {
    if(cardIndex === (context.length -1)){setCardIndex(0)}
    else {
    setCardIndex(cardIndex +1)    
  }
  if(!flip) {
...
const handleScoreState = () => {setScore(score + 1)} 
  ...
  <path d="M9.01223 13.2654C6.26518 11.5486 3.67094 9.61903 1.22952 7.47668C0.665609 6.99988 1.00678 5.97618 1.62861 5.71391C4.59154 4.38673 7.21321 2.49221 9.76505 0.475452C10.516 -0.131161 11.4132 0.869112 10.7242 1.55411C8.69545 3.55589 6.41103 5.2003 4.01859 6.77566C6.03443 8.4132 8.17279 9.93502 10.3797 11.3066C11.6682 12.0913 10.3008 14.0501 9.01223 13.2654Z" fill="#1F1F1F"/>
    ...
    <Card handleScoreState={handleScoreState} nextCard={nextCard} setFlip={setFlip} flip={flip} handleFlipState={handleFlipState} tags={context[cardIndex].tags} english={context[cardIndex].english} translation={context[cardIndex].translation} transcription={context[cardIndex].transcription}/>
